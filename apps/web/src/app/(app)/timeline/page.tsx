...
function TimelineCard({ entry }: { entry: TimelineEntry }) {
  const dotColor = DOT_COLORS[entry.eventType] ?? '#1a3560';
  return (
    <div className="relative mb-5 pl-8 last:mb-0">
      ...
      <div className="rounded border p-3 transition"
        ...
        onMouseLeave={(e) => (e.currentTarget.style.borderColor = '#0f2444')}>
        <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
          <span className="text-[0.85rem] font-semibold capitalize" style={{ color: '#ddeeff' }}>
            {entry.eventType.replace(/_/g, ' ')}
          </span>
          <span className="font-mono text-[0.68rem]" style={{ color: '#4d7ab5' }}>
            {fmtDate(entry.occurredAt)}
          </span>
        </div>
        <div className="flex flex-wrap gap-3 text-xs" style={{ color: '#4d7ab5' }}>
          {entry.department && <span>Dept: <span style={{ color: '#ddeeff' }}>{entry.department}</span></span>}
          ...
          {entry.triggeredByUser && <span>By: <span style={{ color: '#ddeeff' }}>{entry.triggeredByUser}</span></span>}
          {entry.sourceSystem && <span>Source: <span style={{ color: '#ddeeff' }}>{entry.sourceSystem}</span></span>}
        </div>
            ...
            {Array.from(new Set(data.map((e) => e.newState).filter(Boolean))).map((stage, i, arr) => (
              <div key={stage} className="flex items-center gap-1">
                <span className="whitespace-nowrap rounded px-2 py-1 text-[0.7rem] font-medium"
                  style={{ background: 'rgba(0,102,204,0.15)', color: '#4DA6FF' }}>
                  {stage}
                </span>
                {i < arr.length - 1 && <span style={{ color: '#1a3560' }}>→</span>}
              </div>
            ))}
          </div>
