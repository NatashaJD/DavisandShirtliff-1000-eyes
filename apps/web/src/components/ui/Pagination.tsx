...
interface PaginationProps { page: number; pageSize: number; total: number; onChange: (p: number) => void; }
      ...
      {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
        ...
        return (
          <button key={p} onClick={() => onChange(p)}
            ...
            style={active
              ? { background: '#0066CC', borderColor: '#0066CC', color: '#ffffff' }
              : { background: '#ffffff', borderColor: '#c8dff5', color: '#2e6fa8' }}
            onMouseEnter={(e) => !active && (e.currentTarget.style.borderColor = '#0066CC')}
            onMouseLeave={(e) => !active && (e.currentTarget.style.borderColor = '#c8dff5')}>
            {p}
          </button>
        );
      })}
